...
class CallbackClient:
    ...
    def __handle_cache_message(self, requests_timeout: int):
        ...
        while attempts>0:
            with session.get(f'http://{self.host}:{self.port}/anticaptcha/cache/{self.task_id}') as resp:
                json_body = resp.json()
    ...
    def captcha_handler(self, requests_timeout: int = 1, auth_params: dict = None):
        ...
        if not result:
            result = {'taskId': self.task_id, 'message': {'taskId': self.task_id, 'status': 'processing'}}
