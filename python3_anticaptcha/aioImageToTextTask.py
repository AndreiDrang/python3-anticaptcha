...
class ImageToTextTask:
	...
	def captcha_handler(self, captcha_link):
		...
		if self.save_format == 'const':
			captcha_id = self.loop.run_until_complete(self.image_const_saver(captcha_link))
		elif self.save_format == 'temp':
			captcha_id = self.loop.run_until_complete(self.image_temp_saver(captcha_link))
		else:
		...
		anticaptcha_answer = self.loop.run_until_complete(self.aio_captcha_handler())
		self.loop.close()
		
		return anticaptcha_answer
